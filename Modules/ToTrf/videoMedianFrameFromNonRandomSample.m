function [median_frame,median_abs_diff_frame]=videoMedianFrameFromNonRandomSample(input_file_name,max_n_frames_to_sample)
...
videoHeader=fnReadVideoInfo(input_file_name);
%vr=VideoReader(input_file_name);
%info=get(vr);
n_frames=videoHeader.m_iNumFrames;
%n_frames=info.NumberOfFrames;
if n_frames==0
  error('Video file %s says it has zero frames',input_file_name);
end

% get the frame size
first_frame=fnReadFrameFromVideo(videoHeader,1);
...
for i=1:n_to_sample
  ...
  sample_frames(:,:,i)=double(fnReadFrameFromVideo(videoHeader,i));
end
